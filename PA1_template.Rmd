...
The plot shows a roughly normal distribution among the total number of steps taken in a day, with a few more "low-activity" days than might be expected, and two days of more than 20,000 steps. Our individual's mean number of steps per day is `r round(mean(steps_per_day$steps), 2)` with median `r median(steps_per_day$steps)`.
...
The plot shows almost no activity for the first 500-600 intervals, with a large spike in activity occurring around interval 800 and lasting until shortly after interval 900; the peak activity occurs at interval `r with(steps_per_interval, interval[steps == max(steps)])` (the mean number of steps taken at this time is `r round(max(steps_per_interval$steps), 2)`). Activity then fluctuates until around interval 1900, wen it begins to drop down to nearly nothing by the end of the day.
...
If we look at the actual data set, some intervals have no recorded data (`r sum(is.na(activity$steps))` intervals, to be exact). In order to get a more complete picture of our individual's day, we will replace missing values during any given interval with the median number of steps taken for that interval over all days where we have data.
